...
# G0-002-D — Move any count-only behavior behind an explicit legacy pack version adapter with separate tests

## Atomic goal

Move any count-only behavior behind an explicit legacy pack version adapter with separate tests.
...
5. Implement exactly this deliverable: Move any count-only behavior behind an explicit legacy pack version adapter with separate tests.
...
```text
g0-002-d: move any count only behavior behind an explicit legacy pack
```
