...
# G0-002-B — Remove the current numeric count-only __velquFunctions fallback from WorkerInner::load

## Atomic goal

Remove the current numeric count-only __velquFunctions fallback from WorkerInner::load.
...
5. Implement exactly this deliverable: Remove the current numeric count-only __velquFunctions fallback from WorkerInner::load.
...
```text
g0-002-b: remove the current numeric count only velqufunctions fallbac
```
