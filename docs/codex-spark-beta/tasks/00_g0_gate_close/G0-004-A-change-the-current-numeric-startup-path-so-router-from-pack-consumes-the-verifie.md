...
# G0-004-A — Change the current numeric startup path so Router::from_pack consumes the verified serialized nodes directly and never calls Router::build

## Atomic goal

Change the current numeric startup path so Router::from_pack consumes the verified serialized nodes directly and never calls Router::build.
...
5. Implement exactly this deliverable: Change the current numeric startup path so Router::from_pack consumes the verified serialized nodes directly and never calls Router::build.
...
```text
g0-004-a: change the current numeric startup path so router from pack
```
