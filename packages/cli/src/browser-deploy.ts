...
` : ""}
  ...
  const host = new WorkerHost(sessionId, () => {
    ...
    // BWASM-Q-007 (D5): capability implementations live INSIDE the Worker
    // (functions cannot cross postMessage); worker.js constructs the same
    // policy-bounded graph from the build-time fetch allowlist.
    return worker;
  });
...
function workerEntrySource(fetchAllowlist: ReadonlyArray<string>): string {
  return `\
...
import { createBrowserCapabilityGraph, ringBufferSink } from "@velqu/browser-runtime";
${workerBootstrapSource()}
// BWASM-Q-007 (D5): capability implementations cannot cross postMessage
// (functions are not structured-cloneable), so the Worker constructs the
// SAME policy-bounded C-001 graph the page uses — build-time constants
// only (fetch allowlist below), no ambient authority beyond it. Handlers
// reach exactly the grants their route declares via ctx.native.<grant>.
const capabilityGraph = createBrowserCapabilityGraph({
  consoleSink: ringBufferSink(128),
  fetchPolicy: { allowedOrigins: ${JSON.stringify(fetchAllowlist)} },
});
...
export async function composeBrowserDeployment(opts: ComposeOptions): Promise<ComposeResult> {
  ...
  writeFileSync(join(browserDir, "worker.js"), workerEntrySource(opts.fetchAllowlist ?? []));
  writeFileSync(
