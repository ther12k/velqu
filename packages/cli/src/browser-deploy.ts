...
function pageEntrySource(basePath: string): string {
  return `\
...
function serviceWorkerEntrySource(basePath: string): string {
  return `\
// @generated by velqu build --target browser-wasm (BWASM-B-005) — do not edit
// B-004 composition, asset/offline lane only. The SW NEVER executes
// handlers — ADR-0037 keeps handler execution in the page's isolated
// Worker; SW-served API execution is out of MVP scope (BWASM-B-006/Q-002
// own the real-browser verification lanes).
import {
  buildCachePlan,
  cacheNameFor,
  handleFetchEvent,
...
let manifest = null;   // BrowserArtifactManifest once installed
let cache = null;      // content-addressed cache for this buildId

async function ensureCache() {
  if (!manifest) manifest = await (await fetch(MANIFEST_URL)).json();
  if (!cache) cache = await caches.open(cacheNameFor(manifest.appId, manifest.buildId));
  return cache;
}

self.addEventListener("install", (event) => {
  event.waitUntil((async () => {
    const cacheApi = await ensureCache();
    for (const entry of buildCachePlan(manifest, BASE_URL)) {
      const response = await fetch(entry.url);
      if (!response.ok) throw new Error("velqu sw install: prefetch failed " + entry.url);
      await cacheApi.put(entry.url, response.clone());
    }
  })());
});
...
self.addEventListener("activate", (event) => {
  event.waitUntil((async () => {
    // Kill-and-replace per build: caches of other buildIds are stale.
    const keep = cacheNameFor(manifest.appId, manifest.buildId);
    for (const name of await caches.keys()) {
      if (name.startsWith("velqu:" + manifest.appId + ":") && name !== keep) {
...
self.addEventListener("fetch", (event) => {
  if (!manifest) return; // before install completes: pure passthrough
  handleFetchEvent(event, {
    scope: SCOPE,
    locationOrigin: self.location.origin,
    manifest,
    runtimeFetch: (request) => fetch(request),
    cache: {
...
export async function composeBrowserDeployment(opts: ComposeOptions): Promise<ComposeResult> {
  ...
  writeFileSync(join(browserDir, "page.js"), pageEntrySource(basePath));
  writeFileSync(join(browserDir, "worker.js"), workerEntrySource());
  writeFileSync(join(browserDir, "service-worker.js"), serviceWorkerEntrySource(basePath));
  writeFileSync(
  ...
  await bundleEntries(
    browserDir,
    ["page.js", "worker.js", "service-worker.js", "app.bundle.js"],
