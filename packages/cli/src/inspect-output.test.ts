...
describe("Inspect output fidelity (M4A-006-D)", () => {
  ...
  it("reports actual strategy distribution rather than route-count placeholders", async () => {
    ...
    const output = JSON.parse(result);
    expect(output.routeCount).toBe(9);
    expect(output.strategyDistribution.nativeValidationRoutes + output.strategyDistribution.jsValidationRoutes).toBe(9);
    expect(output.strategyDistribution.nativeResponseRoutes + output.strategyDistribution.jsResponseRoutes).toBe(9);
