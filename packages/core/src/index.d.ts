 ...
 */
import type { Schema } from "@velqu/schema";
export type ProblemId = "validation" | "unauthorized" | "not-found" | "method" | "body" | "limit" | "timeout" | "internal";
...
export declare class Status<S extends number> {
    readonly status: S;
    constructor(status: S);
    value<T>(value: T): OkValue<S, T>;
    problem(problem: ProblemId, opts?: {
        detail?: string;
        errors?: readonly FieldIssue[];
    }): ProblemValue<S>;
}
/** `status(201).value(user)` / `status(404).problem("not-found")` */
export declare function status<S extends number>(code: S): Status<S>;
...
export interface RouteDecl<Id extends string, M extends HttpMethod, P = unknown, Q = unknown, B = unknown, Resp extends Record<number, unknown> = Record<number, unknown>, Sess = undefined> {
    ...
    readonly response: {
        [K in keyof Resp & number]: Schema<Resp[K]>;
    };
    readonly handle: (ctx: HandlerCtx<P, Q, B, Sess>) => Promise<HandlerResult<Resp>> | HandlerResult<Resp>;
}
...
} | ProblemValue<number> | Resp[keyof Resp];
export type RouteParams<R> = R extends {
    readonly params?: Schema<infer P>;
} ? P : Record<string, never>;
export type RouteQuery<R> = R extends {
    readonly query?: Schema<infer Q>;
} ? Q : Record<string, never>;
export type RouteBody<R> = R extends {
    readonly body?: Schema<infer B>;
} ? B : undefined;
export type RouteResponses<R> = R extends {
    readonly response: infer Resp;
} ? Resp : never;
export type RoutePath<R> = R extends {
    readonly path: infer Pa extends string;
} ? Pa : never;
export type RouteMethod<R> = R extends {
    readonly method: infer M extends HttpMethod;
} ? M : never;
export type RouteSession<R> = R extends {
    readonly policy?: PolicyDecl<infer S>;
} ? S : undefined;
/** Route constructor. `def` is read statically by the compiler. */
export declare function route<Id extends string, M extends HttpMethod, P, Q, B, Resp extends Record<number, unknown>, Sess>(def: {
    id: Id;
    method: M;
    path: string;
    params?: Schema<P>;
...
export interface PolicyDecl<Sess> {
    ...
    readonly declares: {
        readonly [S in number]: ProblemId;
    };
    readonly provides: "session";
    readonly check: (req: PolicyRequest) => Promise<{
        session: Sess;
    } | ProblemValue<number>>;
}
export declare function definePolicy<Sess>(def: {
    id: string;
    header: string;
    declares: {
        readonly [S in number]: ProblemId;
    ...
    provides: "session";
    check: (req: PolicyRequest) => Promise<{
        session: Sess;
    } | ProblemValue<number>>;
}): PolicyDecl<Sess>;
...
export declare function defineService<T>(id: string, factory: () => T): ServiceDecl<T>;
...
export declare function defineModule<Id extends string, Routes extends readonly AnyRoute[]>(def: {
    id: Id;
    routes: Routes & readonly AnyRoute[];
...
export declare function defineApp<Id extends string, Modules extends readonly AnyModule[]>(def: {
    id: Id;
    modules: Modules & readonly AnyModule[];
}): AppDecl<Id, Modules>;
/** Flatten an app's routes into a tuple union for contract extraction. */
export type AppRoutes<A> = A extends {
    readonly modules: readonly (infer Ms)[];
} ? Ms extends AnyModule ? Ms["routes"][number] : never : never;
