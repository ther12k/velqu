...
export async function bundleApp(
  ...
  const fnVectorElements: string[] = [];
  for (const r of app.routes) {
    const ns = nsNames.get(r.sourceFile)!;
    const isDefault = r.bindingName === "__default_export";
    fnVectorElements.push(`function (ctx) { return (${ns}.${isDefault ? "default" : r.bindingName}).handle(ctx); }`);
  }
  for (const p of app.policies) {
    const ns = nsNames.get(p.sourceFile)!;
    fnVectorElements.push(`function (req) { return (${ns}.${p.bindingName}).check(req); }`);
  }

  // Direct vector dispatch (M2.3 numeric vector):
  const entry = `// @generated by @velqu/compiler — do not edit\n${imports.join("\n")}\n\nglobalThis.__velquFunctions = [\n  ${fnVectorElements.join(",\n  ")}\n];\n`;
